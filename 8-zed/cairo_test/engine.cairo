...
from cairo_test.constants.constants_jessica import ns_jessica_character_dimension
from cairo_test.body.body import _body
from cairo_test.combo import _combo
from cairo_test.physics import _physicality, _test_rectangle_overlap
from cairo_test.perceptibles import update_perceptibles
from cairo_test.lib.tree import Tree, BinaryOperatorTree
from cairo_test.utils import fill_dictionary_offsets, fill_dictionary

func submit_agent{range_check_ptr}(
...
func _loop{range_check_ptr}(
    ...
    if (agent_0_standing == 0) {
        return (frames_len=idx, frames=arr_frames);
    }
...
func playerInLoop{range_check_ptr}(
    ...
    let agent_0_physics_state = PhysicsState(agent_0_position, agent_0_vel, agent_0_acc);
    ...
    let agent_1_physics_state = PhysicsState(agent_1_position, agent_1_vel, agent_1_acc);

    //
    // Perception Phase
    // (given physics states and body states, produce perceptibles)
    //
    let combo_1 = Combo(combo_index=current_combo_1, action_index=combo_counter_1);
    let p_1 = Perceptibles(
    ...
    tempvar tree = cast(ptr_tree, Tree*);
    let (ptr_offsets) = dict_read{dict_ptr=mental_state_offsets_1_filled}(key=agent_1_mental_state);
    tempvar offsets = cast(ptr_offsets, felt*);
    let (agent_state_1, conditions_1_new, dict_new) = BinaryOperatorTree.execute_tree_chain(
    ...
    let combo_info_1 = RealTimeComboInfo(
        current_combo=combos_1_new.current_combo, combo_counter=combos_1_new.combo_counter
    );
