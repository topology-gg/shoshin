...
use cairo_execution::utils::{convert_to_felt, prepare_args};
use cairo_felt::{self, Felt};
use cairo_vm::vm::vm_core::VirtualMachine;
use num_traits::ToPrimitive;
use types::{ConditionEvaluationInputsVec, ConditionEvaluationOutput};
...
fn get_output(vm: VirtualMachine) -> Result<ConditionEvaluationOutput, Error> {
    ...
    let return_values = vm.get_return_values(1)?;
    ...
    Ok(ConditionEvaluationOutput { evaluation })
}
