...
export const OPERATOR_VALUE: Map<string, number> = new Map(
    Object.entries({
        '+': 1,
        OR: 1,
        '-': 2,
...
export function verifyValidCondition(
    c: Condition,
    confirm: boolean
    ...
    for (let [index, e] of c?.elements.entries()) {
        ...
        let isCurrentSyntaxOpen =
            e?.value == Operator.OpenParenthesis ||
            e?.value == Operator.OpenAbs;
        let isCurrentSyntaxClose =
            e?.value == Operator.CloseParenthesis ||
            e?.value == Operator.CloseAbs;
        let isPrevNumeric =
            prevElement?.type === ElementType.Perceptible ||
            prevElement.type === ElementType.Constant ||
            prevElement.type === ElementType.BodyState;
        if (isPrevNumeric && isCurrentSyntaxOpen) {
            return {
                isValid: false,
                message: `Operator "${e?.value}" must be preceded by an operator`,
                conditionElementIndex: index,
                is_bracket_error: false,
            };
        }
        if (
            !isPrevNumeric &&
            prevElement?.value !== Operator.CloseParenthesis &&
            isCurrentSyntaxClose
        ) {
            return {
                isValid: false,
                message: `Operator "${e?.value}" must be preceded by a constant, perceptible or bodystate`,
                conditionElementIndex: index,
                is_bracket_error: false,
            };
        }
        if (e?.value == Operator.Not && isPrevNumeric) {
            return {
                isValid: false,
                message: `Operator "${e?.value}" must be preceded by operator`,
                conditionElementIndex: index,
                is_bracket_error: false,
            };
        }
        if (prevElement?.value == Operator.Not && !isCurrentSyntaxOpen) {
            return {
                isValid: false,
                message: `Operator "${prevElement?.value}" must be followed by ( or Abs(`,
                conditionElementIndex: index,
                is_bracket_error: false,
            };
        }
        ...
        if (countParenthesis < 0 || countAbs < 0) {
            return {
                isValid: false,
                message: 'Unbalanced parenthesis or absolute value',
                conditionElementIndex: index,
                is_bracket_error: true,
            };
        }
        ...
        switch (e?.type) {
            case ElementType.Operator: {
                ...
                if (
                    ...
                    return {
                        isValid: false,
                        message: `Operator "${e?.value}" must be preceded by a constant, perceptible or closing of absolute/parenthesis`,
                        conditionElementIndex: index,
                        is_bracket_error: false,
                    };
                }
            ...
            case ElementType.BodyState: {
                if (prevElement.type !== ElementType.Operator) {
                    return {
                        isValid: false,
                        message: `${e?.type} must be preceded by an operator`,
                        conditionElementIndex: index,
                        is_bracket_error: false,
                    };
                }
...
export function validateConditionName(
    ...
    if (name.length > 31) {
        return (
            'Name should be less than 31 characters long' +
            `\n String \"${name}\" is ${name.length} characters long!`
        );
    }

    const nameCollision = conditions.find(
        (condition) => condition.displayName == name
    );
...
function parseInner(c: ConditionElement[]): Leaf {
    ...
    if (
        ...
        elem.value === Operator.OpenParenthesis
    ) {
        ...
        if (c[i]) {
            ...
            return {
                value: operator,
                left: parseInner(c.slice(1, i - 1)),
                right: parseInner(c.slice(i + 1)),
            };
        }
        ...
        if (c[i]) {
            ...
            return {
                ...
                left: {
                    value: abs,
                    left: -1,
                    right: parseInner(c.slice(1, i - 1)),
                },
                right: parseInner(c.slice(i + 1)),
            };
        }
...
function parseElement(val: ConditionElement): Leaf {
    ...
    return val?.type === ElementType.Perceptible
        ? { value: 14, left: -1, right: wrapToLeaf(value) }
        : wrapToLeaf(value);
}
...
export const conditionElementToStr = (elem: ConditionElement) => {
    ...
    if (type === ElementType.BodyState) {
        ...
        return value > 1000
            ? 'Antoc' + BodystatesAntoc[value]
            : 'Jessica' + BodystatesJessica[value];
    }
