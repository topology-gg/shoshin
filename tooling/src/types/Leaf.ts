...
export function flattenLeaf(n: Leaf): number[] {
    ...
    return [
        n.value,
        1,
        brancheSize(n.left as Leaf) + 1,
        ...flattenLeaf(n.left as Leaf),
        ...flattenLeaf(n.right as Leaf),
    ];
}
...
export function unflattenLeaf(n: SimpleLeaf[]): Leaf {
    ...
    if (leaf.left == -1) {
        return {
            value: leaf.value,
            left: leaf.left,
            right: unflattenLeaf(n.slice(leaf.right)),
        };
    }
    return {
        value: leaf.value,
        left: unflattenLeaf(n.slice(leaf.left)),
        right: unflattenLeaf(n.slice(leaf.right)),
    };
}
...
export function unwrapLeafToCondition(f: Leaf): ConditionElement[] {
    ...
    if (f.left == -1) {
        ...
        return isPerceptible(f.value)
            ? [
                  {
                      value: (f.right as Leaf).value,
...
export function unwrapLeafToTree(
    f: Leaf,
    msNames: string[],
    ...
    if (f.left == -1 && f.right == -1) {
        return [
            { id: msNames[f.value], isChild: true, branch: Direction.Right },
        ];
    }
    ...
    return [
        { id: conditionIndex.toString(), isChild: false },
