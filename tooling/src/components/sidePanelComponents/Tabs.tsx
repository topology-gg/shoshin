...
const Tabs = ({ workingTab, handleClickTab }) => {
    ...
    return (
        ...
        <Box sx={{ borderColor: 'divider', display:'flex', justifyContent:"flex-start" }}>
            <button onClick={(event) => handleTabChange(event, EditorTabName.Profile)} style={tabStyle(workingTab, EditorTabName.Profile)}>Profile</button>
            <button onClick={(event) => handleTabChange(event, EditorTabName.Mind)} style={tabStyle(workingTab, EditorTabName.Mind)}>Mind</button>
            <button onClick={(event) => handleTabChange(event, EditorTabName.Combos)} style={tabStyle(workingTab, EditorTabName.Combos)}>Combos</button>
            <button onClick={(event) => handleTabChange(event, EditorTabName.Conditions)} style={tabStyle(workingTab, EditorTabName.Conditions)}>Conditions</button>
        </Box>
    );
};
