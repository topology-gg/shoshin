...
func loop{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(len: felt) -> () {
    ...
    _loop(idx=0, len=len, arr=arr, obj_state=ns_object_state.IDLE_0);
...
func _loop{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    idx: felt, len: felt, arr: AgentFrameState*, obj_state: felt
) -> () {
    alloc_locals;
    ...
    let (a) = _agent_stub();
    let (obj_state_nxt) = _compute_object_next_state(state=obj_state, agent_action=a);
