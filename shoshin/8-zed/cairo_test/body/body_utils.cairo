...
from cairo_test.constants.constants import ns_common_stamina_effect, ns_stamina, ns_character_type
from cairo_test.constants.constants_antoc import ns_antoc_stamina_effect, ns_antoc_action
from cairo_test.constants.constants_jessica import ns_jessica_stamina_effect, ns_jessica_action
...
func calculate_default_stamina{range_check_ptr}(stamina: felt, character_type: felt) -> felt {
    if (character_type == ns_character_type.JESSICA) {
        let (updated_stamina_default_recovery, _) = _settle_stamina_change(
            stamina, ns_common_stamina_effect.NULL, ns_stamina.INIT_STAMINA
        );
...
func calculate_stamina_change{range_check_ptr}(
    stamina: felt, intent: felt, max_stamina: felt, character_type: felt
) -> (felt, felt) {
    if (character_type == ns_character_type.JESSICA) {
        if (intent == ns_jessica_action.SLASH) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_jessica_stamina_effect.SLASH, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.UPSWING) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_jessica_stamina_effect.UPSWING, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.SIDECUT) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_jessica_stamina_effect.SIDECUT, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.BLOCK) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.BLOCK, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.DASH_FORWARD) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.DASH_FORWARD, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.DASH_BACKWARD) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.DASH_BACKWARD, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.MOVE_FORWARD) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.MOVE_FORWARD, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.MOVE_BACKWARD) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.MOVE_BACKWARD, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.JUMP) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.JUMP, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_jessica_action.NULL) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_common_stamina_effect.NULL, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
    }
    ...
    if (character_type == ns_character_type.ANTOC) {
        if (intent == ns_antoc_action.HORI) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_antoc_stamina_effect.HORI, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        if (intent == ns_antoc_action.VERT) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_antoc_stamina_effect.VERT, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
        ...
        if (intent == ns_antoc_action.STEP_FORWARD) {
            let (updated_stamina, enough_stamina) = _settle_stamina_change(
                stamina, ns_antoc_stamina_effect.STEP_FORWARD, max_stamina
            );
            return (updated_stamina, enough_stamina);
        }
...
func _settle_stamina_change{range_check_ptr}(
    stamina: felt, stamina_change: felt, max_stamina: felt
) -> (felt, felt) {
    alloc_locals;
