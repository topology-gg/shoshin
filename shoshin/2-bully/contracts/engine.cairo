...
from contracts.object import _compute_object_next_state
from contracts.physics import _test_rectangle_overlap

func _agent_stub{range_check_ptr}() -> (agent_action: felt) {
    return (ns_action.PUNCH,);
}

func _stimulus_stub{range_check_ptr}() -> (stimulus: felt) {
...
func _loop{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    idx: felt, len: felt, arr_frames: Frames*
) -> () {
    alloc_locals;
    ...
    let (object_state_0) = _compute_object_next_state(
        state=last_frame.agent_0.object_state, stimulus=s_0, agent_action=a_0
    );
    let (object_state_1) = _compute_object_next_state(
        state=last_frame.agent_1.object_state, stimulus=s_1, agent_action=a_1
    );
