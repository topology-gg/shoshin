...
from contracts.constants import (
    ns_dynamics, ns_object_state, ns_character_dimension,
    Vec2, CharacterState
)
from contracts.numerics import (
    mul_fp, div_fp_ul
)
...
func _euler_forward_no_hitbox {range_check_ptr} (
    ...
    if sign_vel_x == 1:
        assert acc_fp_x = -100 * ns_dynamics.MOVE_ACC_FP
    else:
        if sign_vel_x == -1:
            assert acc_fp_x = 100 * ns_dynamics.MOVE_ACC_FP
        else:
    ...
    #
    update_vel:
    let (vel_fp_nxt : Vec2) = _euler_forward_vel_no_hitbox (
    ...
    #
    let (pos_nxt : Vec2) = _euler_forward_pos_no_hitbox (
        character_state.pos,
        vel_fp_nxt
    )
...
func _euler_forward_vel_no_hitbox {range_check_ptr} (
    ...
    let (vel_fp_nxt_x) = _cap_fp (
        vel_fp.x + delta_vel_x,
        ns_dynamics.MAX_VEL_FP,
        ns_dynamics.MIN_VEL_FP
    )
...
func _euler_forward_pos_no_hitbox {range_check_ptr} (
        pos : Vec2,
        vel_fp : Vec2
    ) -> (
        pos_nxt : Vec2
...
func _euler_forward_consider_hitbox {range_check_ptr} (
        ...
        character_state_fwd_0 : CharacterState,
        character_state_fwd_1 : CharacterState
    ):
    alloc_locals
    ...
    if bool_body_overlap == 1:
        ...
        let (abs_relative_vx_fp) = abs_value (character_state_cand_0.vel_fp.x - character_state_cand_1.vel_fp.x)
        ...
        let (back_off_x_0, _) = signed_div_rem (back_off_x_0_scaled, ns_dynamics.SCALE_FP * ns_dynamics.SCALE_FP, ns_dynamics.RANGE_CHECK_BOUND)
        let (back_off_x_1, _) = signed_div_rem (back_off_x_1_scaled, ns_dynamics.SCALE_FP * ns_dynamics.SCALE_FP, ns_dynamics.RANGE_CHECK_BOUND)
        ...
        return (
            character_state_fwd_0,
            character_state_fwd_1
        )
    else:
        return (
            character_state_cand_0,
            character_state_cand_1
        )
    end
end