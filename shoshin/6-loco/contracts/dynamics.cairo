...
func _euler_forward_no_hitbox{range_check_ptr}(
    ...
    let (vel_fp_nxt: Vec2) = _euler_forward_vel_no_hitbox(
        character_state.vel_fp, Vec2(acc_fp_x, acc_fp_y)
    );
...
func _euler_forward_vel_no_hitbox{range_check_ptr}(vel_fp: Vec2, acc_fp: Vec2) -> (
    vel_fp_nxt: Vec2
) {
    alloc_locals;
    ...
    let (vel_fp_nxt_x) = _cap_fp(
        vel_fp.x + delta_vel_x, ns_dynamics.MAX_VEL_FP, ns_dynamics.MIN_VEL_FP
    );
